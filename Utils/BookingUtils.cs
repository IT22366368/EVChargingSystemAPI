...
namespace SparkPoint_Server.Utils
{
    ...
    public static class BookingFilterUtils
    {
        ...
        //     );
        // }
        //  Returns a filter to get only active bookings (excluding slot-freeing statuses)
public static FilterDefinition<Booking> GetActiveBookingsFilter()
{
    var slotFreeingFilter = Builders<Booking>.Filter.In(
        b => b.Status,
        BookingStatusConstants.SlotFreeingStatuses
    );

    return Builders<Booking>.Filter.Not(slotFreeingFilter);
}
